...
public class ConvexHullTrack implements Track {
    ...
    public void buildTrack() {
        ...
        ArrayList<SVector> rSet = new ArrayList<>();
        ...
        for (int i = 0; i < hull.size(); i++) {
            ...
            SVector disp = new SVector(0, 1)
                    .rotate(Math.random() * 2 * Math.PI)
                    .withMag(dispLen);
            SVector p1 = hull.get(i);
            SVector p2 = hull.get((i + 1) % hull.size());
            SVector mid = SVector.add(p1, p2).mult(0.5)
                    .add(disp);
            rSet.add(p1);
            rSet.add(mid);
        }
        ...
        SVector center = new SVector(0.5, 0.5);
        for (int i = 0; i < hull.size(); i++) {
            SVector p = hull.get(i);
            SVector v = SVector.sub(p, center)
                    .withLimit(halfTrackWidth);
            SVector inner = SVector.add(v, p);
            v = v.rotate(Math.PI);
            SVector outer = SVector.add(v, p);
            ptsInner.add(inner);
            ptsOuter.add(outer);
    ...
    private ArrayList<SVector> getConvexHull(ArrayList<SVector> points) {
        ...
        SVector nextVertex = points.get(1);
        int index = 2;
        boolean finished = false;
        while (!finished) {
            SVector checking = points.get(index);
            SVector a = SVector.sub(nextVertex, currentVertex);
            SVector b = SVector.sub(checking, currentVertex);
    ...
    private ArrayList<SVector> smoothTrack(ArrayList<SVector> dataSet) {
        Vector2[] data = new Vector2[dataSet.size()];
        for (SVector p : dataSet) {
            data[dataSet.indexOf(p)] = new Vector2((float) p.x(), (float) p.y());
        }
        float step = 1 / 4000f;
        ArrayList<SVector> smoothed = new ArrayList<>();
        for (float t = 0; t <= 1.0f; ) {
            ...
            v1.set(p).add(deriv);
            smoothed.add(new SVector(v1.x, v1.y));
            Vector2 v2 = new Vector2();
            v2.set(p).sub(deriv);
            smoothed.add(new SVector(v2.x, v2.y));
            t += step / len;
        }
        return smoothed;
    }

    private void pushApart(ArrayList<SVector> points) {
        double dst = 15 / 400d;
        double dst2 = dst * dst;
        for (int i = 0; i < points.size(); i++) {
            for (int j = i + 1; j < points.size(); j++) {
                if (dst2(points.get(i), points.get(j)) < dst2) {
                    ...
                    hx *= dif;
                    hy *= dif;
                    points.get(j).add(new SVector(hx, hy));
                    points.get(i).sub(new SVector(hx, hy));
                }
            }
        }
    }
    ...
    private double dst2(SVector p1, SVector p2) {
